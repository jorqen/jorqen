...
_UNTOUCHED = ("— пишет модель", "<решает модель>")
...
# Раздел, внутри которого лежит письмо. Он же обязателен для карточки:
# человек открывает её, чтобы откликнуться, и ему нужны и контакт, и текст.
LETTER_HEADING = "### Отклик"
...
# Служебные строки раздела «Отклик»: пояснение курсивом, цитата, команда,
# и адрес «**Куда писать: …**» — он часть карточки, а не письма. Принять
# его за письмо значит получить «3 слова, markdown» на каждой карточке,
# где текст ещё не собран (09.08.2026).
_SKELETON_LINE = re.compile(r"^\s*(?:_|`|>|\*\*Куда писать)")
...
def letter_of(text: str) -> str:
    ...
    # 🔴 Нет блока — нет и письма. Раздел «Отклик» содержит ещё и адрес, куда
    # писать («**Куда писать: …**»), и пояснение про черновик; принять их за
    # письмо значит получить «3 слова, markdown в письме» на каждой карточке,
    # где текст ещё не собран (09.08.2026). Пустой ответ здесь честнее: он
    # означает ровно «письма пока нет».
    if "```" in text:
        return ""
    ...
    got = "\n".join(keep).strip()
    # Одна-две строки письмом не бывают: это остаток служебной разметки.
    return got if len(got.split()) >= 12 else ""
