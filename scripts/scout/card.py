    ...
    if not lines:
        ...
        for para in re.split(r"\n{2,}", text):
            if _REQ_SECTION.search(para):
                ...
                break
    lines = [ln for ln in lines if not _SERVICE_LINE.search(ln)]
    # Псевдопункты: в телеграм-посте весь текст лежит одним куском, и «пунктом»
    # оказывается абзац на двести символов вместе с приветствием компании.
    # Перечисление после «Стек:» в таком случае честнее — оно и есть требования.
    abzats = lines and (sum(len(x) for x in lines) / len(lines)) > 150
    if not lines or abzats:
...
# хэштегов. 🔴 В коротком посте они оказываются ЕДИНСТВЕННЫМ «пунктом», и
# таблица требований заполнялась строкой «Откликнуться через Runello-бот ↓
# #golang #go #remote» — то есть выглядела заполненной, не будучи ею
# (09.08.2026, 30 постов волны).
_SERVICE_LINE = re.compile(
    r"откликнут\w*\s+через|подать\s+заявку\s+через|жми\s+кнопк|"
    r"^\s*#\w+(?:\s+#\w+)*\s*$|бот[ае]?\s*↓|↓\s*#", re.I)

# «Стек: Go, PostgreSQL, Redis» — перечисление технологий через запятую. Для
# короткого поста это и есть весь список требований, просто оформленный не
# списком. Ищем и русские, и английские заголовки.
_STACK_FIELD = re.compile(
    r"(?:стек|технологии|требовани\w*|навыки|skills?|stack|tech(?:nologies)?|"
    r"requirements?)\s*[:：]\s*([^\n]{10,300})", re.I)


def _stack_as_requirements(text: str) -> list[str]:
    """Перечисление после «Стек:» → пункты требований.

    Заведено под телеграм-посты, где всё содержание вакансии — две строки:
    грейд и стек. Без этого карточка выходила с пустой таблицей требований и
    выглядела недоделанной, хотя разбирать там было что.
    """
    out: list[str] = []
    # Разметку снимаем ДО поиска: в постах пишут «**Стек:** Go, Redis», и
    # закрывающие звёздочки стоят сразу после двоеточия — резать по ним значит
    # обрезать перечисление в ноль (поймано сразу после первой правки).
    plain = re.sub(r"\*\*|__", " ", text or "")
    for m in _STACK_FIELD.finditer(plain):
        tail = m.group(1)
        # Хвост после перечисления отрезаем: в посте всё лежит одной строкой,
        # и за «Стек: Go, Redis» сразу идёт «Разработка и поддержка…».
        tail = re.split(r"\s*(?:откликнут|#\w+|↓)", tail, maxsplit=1, flags=re.I)[0]
        tail = re.split(r"(?<=[a-zA-Zа-яА-Я0-9])\.\s", tail, maxsplit=1)[0]
        # Резать по «/» нельзя: CI/CD, JS/TS и React/Vue — это одно название.
        for part in re.split(r"[,;•]", tail):
            item = part.strip(" .·—-*")
            # Элемент стека короткий. Длинная фраза — уже не технология, а
            # начало описания, прилипшее к перечислению.
            if 2 <= len(item) <= 28 and item.count(" ") <= 2 and not item.startswith("#"):
                out.append(item)
    # Повторы убираем, порядок держим: у поста «Go, Redis, Go» иначе выйдет
    # таблица с одним и тем же требованием дважды.
    seen, uniq = set(), []
    for x in out:
        k = x.lower()
        if k not in seen:
...
def build(conn, url: str, *, skills: list[str] | None = None,
    ...
    out.append("")
    # Черновик фита собирает алгоритм из того, что карточка УЖЕ посчитала:
    # совпавшие требования, незакрытые обязательные, барьеры, флаги, условия.
    # Модели остаётся вывод — «писать или пропустить», — и он помечен явно.
    from . import fit as _fit  # noqa: PLC0415

    fit_draft = _fit.draft("\n".join(out))
    if fit_draft:
        out.append("### Фит — черновик, вывод за моделью")
        out.append("")
        out.append(fit_draft)
    else:
    ...
    out.append("")
    # 🔴 Черновик письма собирает АЛГОРИТМ — из фактов резюме под требования
    # этой вакансии. Раньше здесь стояла заглушка «пишет модель», и каждое из
    # восьмидесяти писем волны я писал заново, пересказывая своими словами одни
    # и те же эпизоды. Требование владельца 09.08.2026: «всё, что можно
    # переложить на алгоритм, нужно переложить», агент только правит.
    #
    # Заготовкой с готовыми фразами это не является: подбор идёт по совпадению
    # требований вакансии с эпизодами резюме, и под разные вакансии в письмо
    # попадают разные факты. Выдумать генератор ничего не может — берёт только
    # то, что записано в `resume.yaml`.
    from . import letter as _letter  # noqa: PLC0415 — только ради черновика

    draft = ""
    try:
        resume_data, _why = _resume()
        if resume_data:
            draft = _letter.draft(title=r.get("title") or "", reqs=reqs,
                                  resume=resume_data, url=r.get("url") or "")
    except Exception:  # noqa: BLE001 — резюме не читается: карточка всё равно нужна
        draft = ""

    # Раздел называется «Отклик», а не «Письмо»: человек открывает карточку,
    # чтобы откликнуться, и ему нужны обе вещи сразу — куда писать и что.
    # Линт требует ровно этот заголовок (`cardfiles._NEEDED`), и это не
    # формальность: карточка без него не отвечает на главный вопрос.
    out.append("### Отклик")
    best_route = applyopt.best(opts) if opts else None
    if best_route:
        out.append(f"**Куда писать: {best_route}**")
        out.append("")
    if draft:
        out.append("_Черновик собран по фактам резюме под требования этой "
                   "вакансии. Вычитай: связки, вопрос про барьер и порядок "
                   "абзацев — за тобой. Канон — `references/letter-guide.md`._")
        out.append("")
        out.append("```")
        out.append(draft)
        out.append("```")
    else:
        out.append("_Сопоставлять не с чем: требований в вакансии не нашлось. "
                   "Письмо пишется целиком, правила — `references/letter-guide.md`._")
