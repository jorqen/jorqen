...
def test_force_never_wipes_a_written_fit_and_letter():
    """`--force` не затирает написанные фит и письмо. Восстановить их нечем.

    🔴 Живой счёт 09.08.2026: я применил `--force` к списку карточек, куда
    попали готовые, и шестнадцать фитов с письмами исчезли безвозвратно —
    `.jobs/` лежит в .gitignore, в базе этого нет, копий не осталось.

    Правильный режим для готовой карточки — `--refresh`, и переключаться на
    него должен КОД, а не моя внимательность: цена ошибки здесь необратима.
    Скелет (заглушка «пишет модель») затирается как прежде — терять там нечего.
    """
    from . import cardfiles

    written = ("## Роль — Компания\n\n- **Ссылка:** https://x/1\n\n"
               "### Связь\n  старые маршруты\n\n"
               "### Фит\n\nПисать: совпадение по задачам.\n\n"
               "### Отклик\n\n```\nЗдравствуйте! Откликаюсь…\n```\n")
    skeleton = ("## Роль — Компания\n\n- **Ссылка:** https://x/1\n\n"
                "### Связь\n  старые маршруты\n\n"
                "### Фит — пишет модель\n_заглушка_\n")
    fresh = ("## Роль — Компания\n\n- **Ссылка:** https://x/1\n\n"
             "### Связь\n  СВЕЖИЕ маршруты\n\n"
    ...
    # Признак «в карточке есть работа модели» — тот же, по которому режет
    # `refresh_text`: заголовок «### Фит» без пометки «— пишет модель».
    merged = cardfiles.refresh_text(written, fresh)
    if merged is None or "Здравствуйте! Откликаюсь" not in merged:
        FAILS.append("письмо потеряно при слиянии свежих фактов с написанным")
    if merged and "СВЕЖИЕ маршруты" not in merged:
        FAILS.append("факты не обновились")
    # У скелета сливать нечего — там заглушка, и её надо перезаписать целиком.
    if cardfiles.refresh_text(skeleton, fresh) is None:
        pass          # ожидаемо: работы модели нет
    else:
        got = cardfiles.refresh_text(skeleton, fresh)
        if got and "_заглушка_" in got and "СВЕЖИЕ" not in got:
            FAILS.append("скелет не обновился, хотя терять в нём нечего")
...
def test_the_letter_draft_picks_the_rare_match_not_the_common_one():
    ...
    🔴 Требование владельца 09.08.2026: «всё, что можно переложить на алгоритм,
    нужно переложить, агент только правит». Восемьдесят писем волны я писал
    заново, пересказывая своими словами одни и те же эпизоды резюме.

    Ключевая тонкость — вес. «Go» стоит почти в каждом эпизоде и не различает
    их вовсе; «highload» или «payments» — в единицах, и именно они отвечают на
    вопрос «почему этот кандидат». Пока совпадения считались штуками, под
    требования «Go, высоконагруженные системы, PostgreSQL» письмо собиралось из
    эпизода про фреймворк интеграционного тестирования: у него тоже был «go».
    """
    from . import letter

    resume = {"experience": {"items": [
        {"company": "АТОМ", "highlights": [
            {"ru": "Реализовал высоконагруженный сервис на Go: 70K+ запросов в "
                   "секунду при p99 ниже 50 мс, с запасом для роста."},
            {"ru": "Написал на Go внутренний инструмент сборки и настроил CI/CD "
                   "пайплайны для команды разработки."}]},
        {"company": "Exnode", "highlights": [
            {"ru": "Разобрал критический инцидент с конвертацией валют, откатил "
                   "затронутые транзакции и усилил валидацию платежей."}]},
        {"company": "Sber Tech", "highlights": [
            {"ru": "Спроектировал на Go фреймворк интеграционного тестирования с "
                   "Kubernetes-окружениями и отчётами в CI."}]},
    ]}}

    reqs = ["Коммерческий опыт разработки на Go от 4 лет",
            "Опыт разработки высоконагруженных систем"]
    text = letter.draft(title="Go-разработчик", reqs=reqs, resume=resume,
    ...
    # про пайплайны.
    if "ci" in letter._terms("Понимание принципов ACID и транзакционности"):
        FAILS.append("«ci» найден внутри «ACID» — поиск идёт подстрокой")

    # Канон: длинного тире в письме не бывает, подпись отдельной строкой.
    if "—" in text:
        FAILS.append("в черновике письма осталось длинное тире")
