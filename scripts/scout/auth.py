...
PLATFORMS: dict[str, dict] = {
    ...
    "wantapply": {
        ...
        "cloudflare": True,
        # 🔴 Токен мы НЕ ротируем: у wantapply читаем куку и только. Значит
        # забрать её свежую копию из повседневного браузера безопасно — живая
        # вкладка владельца от этого не разлогинится, в отличие от hirehi, где
        # обмен refresh-токена ротирует единственный креденшл.
        #
        # Отсюда `adopt_safe`: такие площадки обновляются САМИ перед прогоном,
        # без флага и без вопросов. Слова владельца 09.08.2026: «у меня даже не
        # было выхода из аккаунта» — сессия в браузере жила, а прогон видел
        # устаревший слепок и объявлял вход мёртвым, теряя прямые ссылки в ATS.
        "adopt_safe": True,
    },
    "geekjob": {
...
def login(platform: str, *, browser: str | None = None, wait: int = 0,
    ...
    # 🔴 Площадка за управляемым Cloudflare — окна НЕ открываем вовсе.
    #
    # Замер 08.08.2026, три попытки подряд: wantapply.com отдаёт окну под
    # управлением Playwright не проверку, а глухое «Sorry, you have been
    # blocked». Снятие `--no-sandbox` убрало плашку Chrome, но не блок:
    # Cloudflare режет сам факт управляемого браузера. Обходить это мы не
    # будем — это уже уклонение от защиты, а не вход.
    #
    # Зато обходить и не надо: сборщик читает куки ИЗ ПОВСЕДНЕВНОГО браузера
    # (`cookiesrc`), и вход, сделанный руками в Яндексе, доезжает до него сам.
    # Открывать окно, которое заведомо покажет блок, — это отправить человека
    # делать невозможное и потерять его время дважды.
    if cfg.get("cloudflare"):
        print(f"{platform}: у площадки управляемый Cloudflare, и окно под "
              f"управлением скрипта она блокирует наглухо — не проверкой, "
              f"а отказом.")
        print(f"\n  Войди в СВОЁМ обычном браузере: {cfg['login_url']}")
        print("  Сборщик читает куки прямо из браузера — больше ничего делать "
              "не надо.")
        print(f"  Проверить, что дошло: `scout auth status` "
              f"(строка {platform}).")
        if cfg.get("login_gains"):
            print(f"\n  Вход даёт: {cfg['login_gains']}")
        if cfg.get("anon_ok"):
            print(f"  Без входа при этом доступно: {cfg['anon_ok']}")
        return 0
