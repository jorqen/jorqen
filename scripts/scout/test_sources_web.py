...
def test_rabota_skips_two_letter_query():
    ...
    Проверяется именно ОТСУТСТВИЕ короткого запроса среди отправленных, а не
    пустота списка. Раньше это было одно и то же — запрос был один. С
    проверенным набором формулировок (RABOTA_QUERIES) в сеть уходят соседи, и
    «сеть не трогали вовсе» перестало быть признаком: под ним пряталось бы
    «набор площадки не подмешался», то есть тихая потеря выдачи.
    """
    net = _Net({"rabota.ru": RABOTA_HTML})
    ...
    eq([u for u in net.asked if "query=Go&" in u or u.endswith("query=Go")], [],
       "двухбуквенный запрос ушёл в сеть — площадка ответит на него нулём")
    true(any("query=backend" in u for u in net.asked),
         "проверенный набор формулировок не подмешался: без него rabota отдаёт "
         "1 вакансию вместо 13 (замер 08.08.2026)")
