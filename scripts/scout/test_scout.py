...
def test_a_short_post_gives_its_stack_as_requirements():
    ...
    # `description` выжимки. Таблица выходила вдвое длиннее, а счётчик
    # «обязательных без совпадения» врал во столько же раз: у SaltWort стояло
    # «6» там, где на деле 3 (09.08.2026).
    twice = ("Требования:\n- Опыт разработки на Go от 5 лет\n- Знание PostgreSQL\n"
             "Требования:\n- Опыт разработки на Go от 5 лет.\n- Знание PostgreSQL\n")
    got3 = requirements(twice)
    if len(got3) != len(set(x.rstrip(".").lower() for x in got3)):
        FAILS.append(f"требования задвоились: {got3}")
...
def test_a_yearly_figure_is_not_read_as_a_monthly_one():
    """Число, слишком большое для месяца, месячным не считается.

    🔴 Живой счёт 09.08.2026: jooble отдаёт американские вилки без слова о
    периоде («197 300–225 100 USD»), и в шорт-листе они стояли рядом с «от
    240 000 RUB/мес». Человек сравнивает несравнимое, а сортировка по деньгам
    выносит годовые вилки наверх — то есть вытесняет настоящие месячные.

    Догадка строго в одну сторону. «Слишком маленькое — значит час» не делается
    вовсе: маленькая вилка бывает и настоящей (в этом проекте она вообще не
    повод отсеять), а ошибка там дороже.
    """
    from .salary import period_by_magnitude as by_size

    for lo, hi, cur, want in ((197300, 225100, "USD", "year"),
                              (336000, 403200, "PLN", "year"),
                              (50000, 70000, "GBP", "year"),
                              (4000, 6000, "USD", None),     # настоящая месячная
                              (25000, 30000, "PLN", None),
                              (300000, 350000, "RUB", None),
                              (90, 120, "USD", None),        # часовая ставка
                              (5000, 6000, None, None)):     # валюта неизвестна
        eq(by_size(lo, hi, cur), want, f"период по величине для {lo}–{hi} {cur}")

    # Слово в строке первично: сказано «в месяц» — значит месяц, каким бы
    # большим число ни было.
    from .salary import period_from_text
    eq(period_from_text("500 000 ₽ в месяц"), "month",
       "названный период потерян")
