...
def reset_pace() -> None:
    ...
    Нужно потому, что `net.parallel` держит один пул на все источники и
    переиспользует воркеры: без сброса второй источник в том же потоке видит
    паузу первого и платит остаток интервала до своего первого запроса. Их
    площадки при этом разные, и ждать друг друга им незачем — инвариант «перед
    первым запросом ждать нечего» иначе держится только для первого источника
...
class Tally:
    ...
    pages: int = 0
    requests: int = 0
    # Сколько вакансий площадка НАЗВАЛА САМА («найдено 75»). Отдельно от
    # `offered` — того, что реально доехало до разбора, — потому что это два
    # разных вопроса и разные починки. Разрыв между ними означает «мы не
    # спросили» (мало страниц, потолок, стена посреди пагинации), а разрыв
    # внутри `offered` — «мы отфильтровали».
    #
    # Заведено 08.08.2026 после того, как у glassdoor «28 из заявленных 75»
    # пришлось выяснять отдельным расследованием: цифры лежали в сводке порознь
    # и нигде не сходились в одну строку. None — площадка своего числа не
    # называет, и выдумывать его нельзя.
    #
    # 🔴 НЕ ЗАПОЛНЯТЬ У ИСТОЧНИКА С НЕСКОЛЬКИМИ ФОРМУЛИРОВКАМИ суммой по ним.
    # Выдачи формулировок пересекаются («Go» и «Golang» на hh дают общие
    # вакансии), и сумма их `total` больше настоящего числа вакансий. Строка
    # «НЕ СПРОШЕНО 400» из такой суммы — ровно то враньё, ради устранения
    # которого поле и заведено. У многозапросных источников честный отчёт —
    # это отдельное «в выдаче N, взято M» на каждую формулировку, и оно уже есть.
    ...
    def gap_note(self) -> str:
        """Разложить разрыв между «площадка заявила» и «записано». Пусто — нечего.

        Три слагаемых, и они чинятся РАЗНЫМ, поэтому и названы порознь:
        не спросили (мало страниц, потолок, стена посреди пагинации) чинится
        глубиной обхода; чужая профессия и выход за окно — это работа фильтров,
        и чинить там нечего; не разобралось — сменившаяся разметка.
        """
        if self.claimed is None or self.claimed <= self.kept:
            return ""
        parts = []
        unasked = self.claimed - self.offered
        if unasked > 0:
            parts.append(f"НЕ СПРОШЕНО {unasked} (глубина обхода, потолок или "
                         f"стена посреди пагинации)")
        for n, what in ((self.skipped_profile, "чужая профессия"),
                        (self.skipped_old, "старше окна --days"),
                        (self.dupes, "повторы между страницами"),
                        (self.dropped, "не разобралось — разметка"),
                        (self.skipped_kind, "не вакансии")):
            if n:
                parts.append(f"{what} {n}")
        if not parts:
            return ""
        return (f"РАЗРЫВ {self.claimed} → {self.kept}: " + ", ".join(parts))
    ...
    def row(self) -> Vacancy:
        ...
        if self.claimed is not None:
            parts.append(f"площадка заявила {self.claimed}")
        ...
        # Разрыв «заявлено ↔ отдано» раскладывается ЗДЕСЬ, а не в голове
        # читателя: без этой строки в сводке стоят два числа порознь, и
        # «взято 28 при заявленных 75» каждый раз требует расследования —
        # то ли отфильтровали, то ли недоспросили.
        self.note(self.gap_note())
...
#
# ЧЕГО ЗДЕСЬ НЕ ХВАТАЛО: отрицания. Корень `архитектор\w*` нужен ради «Системный
# архитектор» и «Архитектор ИТ», но на государственном портале он тащит другую
# профессию целиком — замер trudvsem 08.08.2026: по слову «архитектор» фильтр
# пропустил 204 строки, профильных среди них 12. Остальное — «Ландшафтный
# архитектор», «Главный архитектор проекта» (ГАП, стройка), «Архитектор-
# проектировщик», «Архитектор по развитию услуг антифрода».
#
# Отрицание стоит ВНУТРИ той же регулярки, а не отдельной функцией рядом: место,
# где спрашивают «наша ли это профессия», в проекте двадцать одно, и вторая
# проверка рядом с первой означает, что девятнадцать из них про неё не узнают.
# `^(?!…)` работает потому, что `search` с якорем пробует единственную позицию.
#
# Голое «Архитектор» НЕ отсекается, хотя на trudvsem таких 50: в IT так называют
# solution-архитекторов, и по одному слову эти два случая не различимы.
# Направление ошибки прежнее — лишняя строка стоит взгляда, пропавшая вакансия
# стоит вакансии.
...
# Формулировки, на которые отвечает ГОСУДАРСТВЕННЫЙ портал. По «Golang» вся база
# страны отдаёт девять записей — не потому, что вакансий нет, а потому, что
# профессию здесь называют иначе.
#
# Замер 08.08.2026, ПРОФИЛЬНЫЕ В ОКНЕ СВЕЖЕСТИ (это и есть то, что попадёт
# в волну; общая глубина базы обманчива — она забита позапрошлогодним):
#
#   окно            3 дн.  7 дн.  14 дн.  30 дн.
#   один «Golang»       0      0       1       1
#   весь набор          5     10      19      36
#
# По формулировкам за 30 дней: системный программист 17 · Python 12 · разработчик 9
# · микросервис 5 · Java 4 · backend 3 · Go 3 · Golang 1.
#
# Цена набора — 19 страниц и ~210 с; портал отвечает по 11 с на запрос, и это
# его скорость, а не наша пауза. Окупается тем, что trudvsem — ЕДИНСТВЕННЫЙ
# источник в проекте, где почта и телефон работодателя приходят из
# государственного реестра, а не добываются ресёрчем.
#
# Чего в наборе НЕТ и почему (замерено, а не додумано):
#   • «программист» — 2161 запись, 22 страницы, 226 секунд, и всего +9 сверх
#     «системного программиста» за 20 секунд. Широкий запрос здесь не полнее,
#     он просто дороже;
#   • «бэкенд» — ноль профильных во всех окнах вплоть до 90 дней;
#   • «инженер-программист» (11 страниц) и «PHP» — ноль новых, целиком внутри
#     остальных;
#   • «архитектор» — 204 прошли фильтр, профильных 12 (см. ATS_ROLE_RE);
#   • «devops» — чистое администрирование не его профиль, это решение владельца,
#     а не свойство площадки;
#   • «software engineer», «программист Go» — площадка отвечает нулём.
TRUDVSEM_QUERIES = ("Go", "backend", "микросервис", "системный программист",
...
def src_trudvsem(ctx: Ctx) -> list[Vacancy]:
    ...
    for q in merge_queries(ctx.queries(), TRUDVSEM_QUERIES):
        total: int | None = None
        for page in range(budget):
            if tally.requests:
                _pause()
    ...
    for row in rows:
        ...
        out.append(Vacancy(
            ...
            raw={"query": q, "path": "api",
                 ...
                 # `contact_list` берётся ОТДЕЛЬНО от `company.email`: там лежит
                 # почта конкретного человека («V.Kulikova@solidbank.ru») и
                 # телефон, а в company.email — общий ящик организации
                 # («bank@solidbank.ru»). Раньше список отбрасывался, и главная
                 # ценность источника доезжала до карточки наполовину.
                 "contact": {k: comp.get(k) for k in ("email", "inn", "hr-agency")
                             if comp.get(k)},
                 "contact_list": [
                     {"type": (c or {}).get("contact_type"),
                      "value": (c or {}).get("contact_value")}
                     for c in (v.get("contact_list") or [])
                     if (c or {}).get("contact_value")],
