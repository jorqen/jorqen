...
PLACEHOLDER_COMPANY = frozenset({
    ...
    # «N/A» и «N.A.» после нормализации распадаются на «n a» — отсюда пара
    # односимвольных токенов. Названий компаний из одной буквы не бывает, а
    # careered отдаёт этим «работодателя» вполне живых вакансий: без запрета
    # карточка уезжала в каталог `companies/n-a/` вместо `_hidden/`, то есть
    # заглушка становилась именем компании прямо в отчёте (08.08.2026).
    "n/a", "n\\a",
