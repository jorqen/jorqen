...
def _src_glassdoor_html(ctx: Ctx) -> list[Vacancy]:
    ...
    # выставляет проверку не всегда, а по нагрузке: замеры 07 и 08.08.2026 дали
    # одно и то же — первая страница проходит, вторая (`_IP2`) упирается.
    #
    # 🔴 Перехват стоит вокруг ПОЛУЧЕНИЯ страницы, а не вокруг `check_wall`, и
    # это не педантизм. Стену видит и бросает САМ рендерер (`render._blocked_or_html`),
    # то есть исключение прилетает из строки `for ... in pages`, а не из тела
    # цикла. Пока `try` обнимал только `check_wall`, защита не срабатывала
    # никогда: 08.08.2026 источник вернул НОЛЬ, выбросив разобранную первую
    # страницу, — при том, что комментарий рядом обещал обратное. Наша
    # собственная проверка тоже остаётся: у неё свой список маркеров, и
    # страница может доехать до тела цикла без отказа рендерера.
    it = iter(pages)
    while True:
        try:
            _asked, html, final = next(it)
