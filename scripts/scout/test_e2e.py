...
def test_everything_collected_is_in_the_db():
    ...
    # Сводки источников (`_summary`) вычитать НЕ надо: `store.upsert` их уже
    # не считает — там `counts = v.external_id != SUMMARY_ID`. Второе вычитание
    # занижало ожидание ровно на число отработавших площадок, и живой прогон
    # 05.08.2026 упал на «получено 10109, ожидалось 10129» при 20 источниках.
    # Сам счётчик был прав: 10129 = len(uniq) точно.
    eq(res["new"] + res["updated"], len(uniq),
       "новых+обновлённых не сходится с числом уникальных вакансий "
       "(сводки в счётчики upsert не входят вовсе)")
