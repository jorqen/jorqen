...
# Статусы `run_source`, при которых площадка НЕ дала выдачи. Разделены по
# смыслу: «выключено» — это настройка (нет ключа, нет входа, пропущено
# флагом), «сломалось» — это авария (упала, за стеной). Первое чинится
# однажды, второе может пройти само.
_OFF = {"skipped", "no_login"}
_BROKEN = {"error", "blocked"}


def coverage_line(db: str) -> str:
    """Одна строка о полноте последнего обхода. Пустая — прогонов не было.

    🔴 Зачем это в посте. Рутина работает без присмотра, и «0 новых вакансий»
    при развалившемся обходе выглядит ТОЧНО ТАК ЖЕ, как «сегодня тихо». Без
    этой строки молчаливый отказ можно не заметить неделями — а это ровно та
    цена, ради которой весь сборщик и считает свои счётчики вслух.
    """
    try:
    ...
    off = [r for r in rows if r.get("status") in _OFF]
    broken = [r for r in rows if r.get("status") in _BROKEN]
    line = f"обход: {len(ok)} из {len(rows)} площадок"
    if off:
        line += f", выключено {len(off)}"
    if broken:
        # Поимённо: «сломалось 3» не даёт понять, потеряли мы hh или jobicy.
        line += (f", ⚠️ не отработали {len(broken)}: "
                 + ", ".join(sorted(r["source"] for r in broken)))
    if dead := _dead_sessions(rows):
        # Отдельной строкой и только про НАСТРОЕННОЕ. Площадка, вход к которой
        # никогда не заводили, — это настройка, и повторять её каждый день
        # значит приучить не читать пост. А вот сессия, которую владелец
        # положил в секреты и которая протухла, — единственное во всём прогоне,
        # что требует его рук: продлить её в облаке нечем, там нет браузера.
        line += (f"\n🔑 сессия протухла: {', '.join(dead)} — "
                 f"войди локально и повтори `scout auth export`")
    return line


def _dead_sessions(rows: list[dict]) -> list[str]:
    """Площадки, вход к которым БЫЛ настроен секретом, но не сработал.

    Отличаем «сессия умерла» от «вход не заводили»: первое чинится и требует
    внимания, второе — осознанная настройка. Признак настроенности — наличие
    переменной окружения, а не файла: файл в облаке кладём мы же из неё.
    """
    from .auth import env_var  # noqa: PLC0415 — auth тянет список площадок

    return sorted(r["source"] for r in rows
                  if r.get("status") in _OFF | _BROKEN
                  and os.environ.get(env_var(r["source"])))
...
def build(db: str, *, days: int, date: str, top: int = 10,
    ...
    # Полнота обхода идёт ВТОРОЙ строкой, до всего остального, и в обеих ветках
    # — включая «ничего нового». Именно там она и нужна: развалившийся обход
    # выглядит как тихий день, и без этой строки отличить их нельзя.
    cover = coverage_line(db)
    if cover:
        head.append(cover)
    if n:
        ...
        head.append("Ничего нового. Либо окно узкое, либо всё уже отработано — "
                    "сверься со строкой обхода выше, прежде чем верить нулю.")
