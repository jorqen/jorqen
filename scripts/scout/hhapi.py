...
# Здесь именно свой UA с почтой владельца, а не подделка под мобильный клиент:
# на выдачу токена UA не влияет (проверено — 502 приходит с любым), а
# притворяться устройством, которого нет, значит врать без выгоды.
DEFAULT_UA = "jorqen-scout/1.0 (job search for own use; matveisisov@gmail.com)"
