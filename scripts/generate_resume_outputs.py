...
# Логотипы, у которых светлый и тёмный варианты — РАЗНЫЕ рисунки, а не один
# в двух цветах: у vstu это перекрашенный герб, у github и sbertech — марка
# бренда, которая на тёмном фоне обязана выглядеть иначе. Их по-прежнему
# подменяет JS (см. app.js), потому что подкрасить их одним цветом нельзя.
THEMED_MEDIA_FILES = {
    "exnode.svg",
    "github.svg",
    "sbertech.svg",
...
# Одноцветные иконки интерфейса. Раньше каждая лежала ДВАЖДЫ — в `light/`
# и `dark/`, — и отличались копии ровно одним значением `stroke`; менял их
# на лету JS. Двадцать файлов на десять рисунков, лишний список в данных
# страницы и двадцать четыре отдельных запроса за картинками.
#
# Теперь рисунок один, цвет ему задаёт страница через `currentColor`, и для
# этого он вставляется в разметку ИНЛАЙНОМ: у `<img>` свой документ, и `color`
# страницы внутрь него не проникает. Побочно: тема к ним применяется без JS
# вообще, а запросов за иконками не остаётся ни одного.
INLINE_ICONS = {
    "briefcase.svg",
    "contact.svg",
    "download.svg",
...
def load_yaml_mapping(path: Path, *, empty_ok: bool = False) -> dict[str, Any]:
    """YAML, который обязан быть словарём верхнего уровня.

    Имя обещало mapping, а отдавалось что угодно: пустой файл давал None,
    список — список, и падало это уже у потребителя, не называя ни файла, ни
    беды. `empty_ok` — для накладки: «файл завёл, содержимое потом» законно и
    означает «накладки нет», а вот список или скаляр — по-прежнему ошибка.
    """
    ...
    if not isinstance(value, dict):
        raise ValueError(f"{path}: ожидался словарь верхнего уровня, "
                         f"а файл дал {type(value).__name__}")
    return value
...
def is_phone_number(value: str) -> bool:
    """Похоже на телефон: плюс, потом только цифры и разделители, цифр 7 и больше."""
    return bool(re.fullmatch(r"\+[\d\s()\-]{6,}", value)) and sum(c.isdigit() for c in value) >= 7
...
def contact_href(contact: dict[str, str]) -> str | None:
    ...
    if is_phone_number(value):
        # Без пробелов и скобок: `tel:` их допускает, но часть читателей — нет.
...
def download_section_title(source: dict[str, Any], lang: str, key: str) -> str:
    """Название раздела в файле: `downloadTitle`, а если его нет — сайтовый `title`.

    Разведены они потому, что аудитории разные. Парсер опознаёт раздел по
    слову в заголовке, и словарь у него короткий: разбор открытого парсера
    резюме (21.08.2026) требует, чтобы заголовок содержал корень
    experience / education / skill / project / summary. «Professional Profile»
    над обычным summary в этот словарь не попадает, а «Профессиональный опыт»
    — не то, что ищет русский парсер, привыкший к «Опыту работы». На сайте же
    человеку показываются прежние названия, и заводить им вторую копию текста
    ради этого не нужно: оба живут в одном `resume.yaml`.
    """
    section = source[DOWNLOAD_SECTION_SOURCE_KEYS[key]]
    title = section.get("downloadTitle") or section["title"]
    return localized_tree(title, lang, source["languages"])
...
def experience_header_text(item: dict[str, Any], labels: dict[str, str], lang: str) -> str:
    """Заголовок места работы для ВЫГРУЗОК — всё, кроме названия компании.

    Формат: `Должность | даты · длительность · локация · тип занятости`.
    Читается так: до первой вертикальной черты — должность, после неё —
    диапазон дат, дальше через `·` — уточнения.

    🔴 Раньше было `Должность (даты, длительность)`, а локация и тип занятости
    вообще не были текстом: в PDF они рисовались отдельными плашками, и
    извлекатель отрывал их от заголовка (замер 21.08.2026 — семь строк-сирот
    на язык, причём русская «ТК РФ» выглядела как заголовок раздела). Внутри
    скобок с датами стояла ещё и длительность, поэтому содержимое скобок не
    было диапазоном дат целиком. Обе беды лечит один разделитель — текстовый.

    На сайт это не влияет: он строит заголовок сам из `format_period` и
    `experience_tag_values`.
    """
    parts = [format_period(item, labels, lang), experience_duration(item, lang)]
    parts.extend(experience_tag_values(item))
    parts.extend(download_site_url(item))
    return f"{item['role']} | {' · '.join(parts)}"
...
def download_site_url(item: dict[str, Any]) -> list[str]:
    """Адрес сайта работодателя или вуза — ВИДИМЫМ текстом, а не гиперссылкой.

    🔴 Замер 21.08.2026: в EN PDF было 11 аннотаций `/URI`, в DOCX — 10 внешних
    Target, а в тексте адресов не было ни одного. Ссылки существовали для
    человека с мышкой и не существовали для парсера, который читает текст.
    Стоит это дёшево: адрес приписывается к заголовку записи и не занимает
    отдельной строки.
    """
    url = item.get("url")
...
def education_header_text(item: dict[str, Any], labels: dict[str, str], lang: str) -> str:
    """Запись об образовании для ВЫГРУЗОК: `даты · пометка | Степень`.

    🔴 Раньше было `Степень (даты)`, причём диапазон заворачивался в скобки
    поверх скобок, уже стоящих в самой степени («(part-time)», «(заочно)»), а
    внутрь диапазона попадало «Ожидаемое окончание: 2030» — то есть скобки
    переставали быть датами.

    Даты стоят ПЕРЕД степенью, и это не косметика. Русское название степени
    длиннее английского на треть, строка не влезает в ширину полосы и рвётся —
    а рвётся она всегда по последнему, что в неё не поместилось. Пока даты шли
    последними, именно они уезжали на отдельную строку и теряли связь с вузом
    (замер 21.08.2026 на RU PDF). Теперь перенос может разорвать только текст
    степени, а пара «вуз ↔ срок обучения» — то, ради чего парсер сюда смотрит,
    — остаётся на одной строке при любой длине названия.

    Сайтовый `format_education_period` не трогается — он показывает человеку.
    """
    period = f"{format_resume_date(item['startDate'], lang)} - {format_resume_date(item['endDate'], lang)}"
    if is_expected_education(item):
        period = f"{period} · {labels['expectedGraduation']}"
    return " · ".join([f"{period} | {item['degree']}", *download_site_url(item)])
...
LOCAL_OVERLAY_NAME = "resume.local.yaml"


def merge_overlay(base: dict[str, Any], overlay: dict[str, Any]) -> dict[str, Any]:
    """Наложить накладку на источник: словари вглубь, всё остальное — заменой."""
    for key, value in overlay.items():
        current = base.get(key)
        if isinstance(current, dict) and isinstance(value, dict):
            merge_overlay(current, value)
        else:
            base[key] = value
    return base
...
def load_data(path: Path) -> dict[str, Any]:
    """Источник резюме плюс необязательная личная накладка рядом с ним.

    🔴 Накладка нужна ровно для одного: репозиторий ПУБЛИЧНЫЙ (инвариант 4), а
    форма отклика почти любого ATS спрашивает телефон. Класть телефон в
    `resume.yaml` значит опубликовать его на GitHub Pages, не класть — оставить
    поле отклика пустым при каждом отклике.

    Поэтому `resume/resume.local.yaml` (в `.gitignore`) накладывается на
    источник ПЕРЕД проверкой схемой — то есть личная часть подчиняется тем же
    правилам, а не живёт вне их. Единый источник истины не раздваивается:
    накладка не переопределяет то, что уже есть в `resume.yaml`, она дописывает
    то, чего там нет и быть не должно. Файла нет — сборка идёт как раньше;
    файл есть, а содержимого в нём ещё нет — тоже (это «накладки нет», а не
    повод уронить сборку). Не-словарь в накладке — ошибка с именем файла.

    Как владельцу включить телефон:

        # resume/resume.local.yaml
        contacts:
          phone:
            value: "+7 900 000-00-00"
    """
    source = load_yaml_mapping(path)
    overlay_path = path.with_name(LOCAL_OVERLAY_NAME)
    if overlay_path.exists():
        merge_overlay(source, load_yaml_mapping(overlay_path, empty_ok=True))
...
def fact_value(source: dict[str, Any], key: str, lang: str) -> str | None:
    """Значение факта по опознавательному ключу — или None, если факта нет.

    Место жительства и формат работы спрашивает почти каждая форма отклика, а
    в файлах их не было вовсе: `person.facts` показывались только на сайте.
    Берутся они отсюда, а не из отдельного поля, чтобы значение осталось в
    одном месте.
    """
    for item in source["person"]["facts"]:
        if item.get("key") == key:
            return localized_tree(item["value"], lang, source["languages"])
    return None
...
def download_profile_facts(source: dict[str, Any], lang: str) -> list[str]:
    """Строки под контактами: где живёт и в каком формате готов работать.

    Отдельными строками, а не через `|` в общем ряду: место жительства — то
    поле, которое форма отклика ищет самостоятельной строкой рядом с
    контактами.
    """
    return [value for value in (fact_value(source, "location", lang),
                                fact_value(source, "format", lang)) if value]
...
def generate_txt(source: dict[str, Any], lang: str, output_path: Path) -> None:
    ...
    for fact_line in download_profile_facts(source, lang):
    ...
    for item in experience_items(source, lang):
        add_txt_line(lines)
        # Адрес сайта компании стоит в самом заголовке (см. download_site_url),
        # отдельной подписанной строки больше нет: три формата обязаны совпадать.
...
# `docx.Document` — фабрика, а не класс: в аннотации она значит «функция»,
# и проверка типов на этом спотыкалась. Тип документа живёт отдельно.
def configure_docx(document: Any, download_style: dict[str, Any], scale: float) -> None:
...
def docx_contact_line(
    ...
    for index, key in enumerate(contact_keys(source)):
        ...
        if index:
            paragraph.add_run("  |  ")
        # Адрес пишется целиком, со схемой: «jorqen.link» без `https://` и без
        # слэша не опознаётся как ссылка ни одной из регулярок открытых
        # парсеров резюме, а `linkedin.com/in/jorqen` — опознаётся. Разницу
        # видно только в тексте, поэтому она и дожила до замера 21.08.2026.
        text = contact_link_text(contact_value_for_lang, shorten_web_urls=False)
        href = contact_href(contact_value_for_lang)
        if href:
            add_hyperlink(paragraph, text, href, accent_color)
        else:
            paragraph.add_run(text)


def generate_docx(source: dict[str, Any], lang: str, output_path: Path, scale: float) -> None:
    profile = person(source, lang)
    label_values = labels(source, lang)
    education = section(source, "education", lang)
    skills = section(source, "skills", lang)
    ...
    for fact_line in download_profile_facts(source, lang):
        document.add_paragraph(fact_line, style="ResumeContact")
...
def download_contacts(source: dict[str, Any], lang: str) -> list[dict[str, Any]]:
    items = []
    for key in contact_keys(source):
        contact_value_for_lang = contact(source, key, lang)
        items.append({
            "key": key,
            # Со схемой — по той же причине, что и в DOCX: см. docx_contact_line.
            "text": contact_link_text(contact_value_for_lang, shorten_web_urls=False),
            "href": contact_href(contact_value_for_lang),
        })
    return items
...
def download_view_model(source: dict[str, Any], lang: str) -> dict[str, Any]:
    profile = person(source, lang)
    label_values = labels(source, lang)
    education = section(source, "education", lang)
    skills = section(source, "skills", lang)
    return {
        ...
        "profile": profile,
        "contacts": download_contacts(source, lang),
        "facts": download_profile_facts(source, lang),
        "sections": {
            "profile": {
...
def render_download_html(source: dict[str, Any], lang: str, scale: float) -> str:
    return render_template(
        "resume_download.html.j2",
        lang=lang,
        model=download_view_model(source, lang),
...
def render_download_pdf_document(source: dict[str, Any], lang: str, scale: float) -> Any:
    return HTML(
        string=render_download_html(source, lang, scale),
        base_url=str(ROOT),
    ).render()


def resolve_download_pdf(source: dict[str, Any], lang: str) -> tuple[float, Any]:
    layout = DOWNLOAD_STYLES["layout"]
    ...
    for _ in range(layout["scaleSearchIterations"]):
        ...
        document = render_download_pdf_document(source, lang, scale)
...
def generate_pdf(source: dict[str, Any], lang: str, output_path: Path) -> float:
    """Собрать PDF выгрузки. Возвращает подобранный масштаб.

    `pdf_tags=True` — не косметика. Без него PDF это набор кусков текста с
    координатами: порядок чтения и роли блоков («заголовок», «пункт списка»)
    извлекатель домысливает по зазорам, и ровно на этом домысле уже рвался
    заголовок места работы. Тегированный файл несёт дерево структуры явно, и
    парсеры на PDFBox читают именно его. Замер 21.08.2026: до правки `pdfinfo`
    отвечал `Tagged: no` на обоих файлах; после — `Tagged: yes`, а извлечённый
    текст, число страниц и размер не изменились.

    Опция появилась в WeasyPrint 61 (в `.venv` и в CI ставится последняя).
    Сторож — `scripts/test_resume_ats.py::test_generated_pdf_is_tagged`.
    """
    scale, document = resolve_download_pdf(source, lang)
    output_path.parent.mkdir(parents=True, exist_ok=True)
    document.write_pdf(str(output_path), pdf_tags=True)
    return scale
...
def inline_icon(file_name: str, css_class: str = "") -> str:
    """Одноцветная иконка прямо в разметке — чтобы работал `currentColor`.

    Через `<img>` не выйдет: у картинки свой документ, и `color` страницы
    внутрь него не проникает. Иконка всегда декоративная: рядом с ней стоит
    текст, который её и называет, поэтому `aria-hidden` и `focusable="false"`
    (в IE/Edge старых версий svg попадал в обход по Tab).
    """
    body = (MEDIA_ROOT / file_name).read_text(encoding="utf-8").strip()
    attrs = ' aria-hidden="true" focusable="false"'
    if css_class:
        attrs += f' class="{html_attr(css_class)}"'
    return _SVG_OPEN_RE.sub(f"<svg{attrs}", body, count=1)
...
def render_icon(
    ...
    *,
    ...
    if file_name in INLINE_ICONS:
        return inline_icon(file_name, css_class)
...
def render_language_switch(source: dict[str, Any], lang: str) -> str:
    ...
    for item_lang in source["languages"]:
        ...
        path = relative_public_path(page_path(item_lang))
        links.append(
...
def render_resume_actions(source: dict[str, Any], lang: str) -> str:
    ...
    for extension in ("pdf", "docx", "txt"):
        ...
        cards.append(
            f"""<a id="resume-{extension}" class="resume-button" href="{html_attr(download_path(lang, file_name))}" download="{html_attr(file_name)}" data-analytics-goal="resume_download_click" data-analytics-label="{extension.upper()} resume" data-analytics-section="resume">
...
# Токены, которые нельзя разрывать переносом строки: слова с дефисом внутри и
# адреса. Дефис — потому, что перенос по нему НЕОБРАТИМ для читателя-парсера:
# извлекатель PDF считает висячий дефис знаком переноса и склеивает половинки
# («Telegram-оповещения» → «telegramоповещения», «AI-assisted» → «aiassisted»).
# Адрес — потому, что `overflow-wrap: break-word` рвёт его в любом месте, и в
# тексте вместо ссылки остаются два огрызка.
_UNBREAKABLE_TOKEN = re.compile(
    r"\S*(?:[0-9A-Za-zА-Яа-яЁё]-[0-9A-Za-zА-Яа-яЁё]|https?://)\S*"
)


def keep_token_whole(value: Any) -> Markup:
    """Обернуть неразрывные токены в `<span class="nobreak">`.

    Почему так, а не иначе. `word-break: keep-all` в WeasyPrint 69.0 перенос по
    дефису НЕ убирает (проверено 21.08.2026). Неразрывный дефис U+2011 убирает,
    но вносит не-ASCII символ ровно в тот текст, ради машинной читаемости
    которого всё и делается. Переписать конкретные фразы — лечение одного
    случая: замер показал, что после первой же правки вёрстки склеек стало не
    одна, а четыре, потому что точка переноса сдвинулась. Общий механизм —
    единственное, что закрывает класс, а не случай.
    """
    escaped = html.escape(str(value))
    return Markup(_UNBREAKABLE_TOKEN.sub(
        lambda match: f'<span class="nobreak">{match.group(0)}</span>', escaped))
...
def template_environment() -> Environment:
    ...
    environment.filters["nobreak"] = keep_token_whole
    return environment
...
def generate_root_resolver_html(source: dict[str, Any]) -> str:
    ...
    page_paths = {lang: relative_public_path(page_path(lang), depth=0) for lang in source["languages"]}
...
def generate_outputs(
    ...
    for lang in source["languages"]:
        ...
        download_scale = generate_pdf(source, lang, lang_dir / file_names["pdf"])
        docx_scale = download_scale * DOWNLOAD_STYLES["layout"]["docxScaleAdjustment"].get(lang, 1.0)
        generate_docx(source, lang, lang_dir / file_names["docx"], docx_scale)
...
def main() -> None:
    ...
    generate_outputs(data, args.output_root)
